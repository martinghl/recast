"""Smoke tests for focal_standalone.py, the single-file mirror of the focal package. It is a plain
top-level script (not part of the installed `focal` package / not guaranteed to be on sys.path under
every pytest invocation mode), so it's loaded directly by file path via importlib rather than a bare
`import focal_standalone`."""
import os
...
_STANDALONE_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                "focal_standalone.py")
_spec = importlib.util.spec_from_file_location("focal_standalone", _STANDALONE_PATH)
focal_standalone = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(focal_standalone)
...
def _leak_fixture():
    ...
    focal_standalone.py's _attribute_one doesn't support (it's zero-baseline only, matching its
    pre-existing behavior -- that support gap is out of scope here). This fixture (seed=0, 5 genes,
    per-gene uniform Poisson rates) was independently grid-searched for focal_standalone's
...
def test_standalone_default_gate_ranks_planted_up_gene_above_sign_mismatch_gene():
    """focal_standalone.py's attribute() default gate must be 'dC' (matching focal/attribute.py):
    ...
    A = _leak_fixture()
    enc = focal_standalone.StubEncoder(A.n_vars)

    # pin the underlying sign mismatch this test relies on
    labels = A.obs["state"].to_numpy()
    tmask, rmask = focal_standalone.resolve_reference(labels, "T", "siblings")
    dC = focal_standalone.pseudobulk_centroid(A.X, tmask) - focal_standalone.pseudobulk_centroid(A.X, rmask)
    dC_by_gene = dict(zip(A.var_names, dC))
    assert dC_by_gene["leak"] < 0 and dC_by_gene["driver"] > 0 and dC_by_gene["driver2"] > 0

    attribution_df, genes_dict, meta = focal_standalone.attribute(
    ...
    _, genes_dict_phi, meta_phi = focal_standalone.attribute(
...
def test_standalone_bad_gate_raises():
    ...
    with pytest.raises(ValueError):
        focal_standalone.attribute(enc, A, "state", target="T", device="cpu", gate="bogus")
