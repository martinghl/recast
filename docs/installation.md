...
RECAST installs in two tiers, and which one you need depends on what you are doing. The split is
...
```bash
pip install git+https://github.com/martinghl/recast.git
```

Pulls only `numpy`, `scipy`, `pandas` and `anndata`. **No torch.** This tier covers
`recast.composite` — re-weighting and re-ranking a panel you already have — plus `recast.io`,
`recast.centroid` and `recast.contrast`. It is what you want on a machine that will never run
an encoder.

## Attribution — selecting genes with an encoder

```bash
pip install "recast[attribution] @ git+https://github.com/martinghl/recast.git"
```
...
- `recast.attribute` and `recast.cluster_attribution`;
- **all** of `recast.score` — the scoring functions import the attribution path at module
  load, so a frozen panel still needs this tier to be scored;
- `recast.contrast_qc`, and every class in `recast.encoders` **including `StubEncoder`**, which
...
RECAST ships no model weights. `SCimilarityEncoder` wants a directory containing
...
extras — set `RECAST_SIGNATURE_SRC` to a checkout if it is not on the path), and `SCVIEncoder`
...
The install puts a `recast` executable on your path:

```bash
recast attribute  --h5ad A.h5ad --encoder scimilarity --model $MODEL --cluster-key label --out attr.h5ad
recast composite  --attr attr.h5ad --h5ad A.h5ad --cluster-key label --out-prefix run
recast score-cells --h5ad B.h5ad --encoder scimilarity --model $MODEL --cluster-key label \
...
```bash
python -c "import recast; print(recast.__version__)"
