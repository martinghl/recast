...
def _attribute_one(enc, counts, target_mask, ref_mask, device, baseline="zero"):
    ...
    C_ref = pseudobulk_centroid(counts, ref_mask)   # needed for dC regardless of the IG baseline choice
    dC = C - C_ref
...
def attribute(enc, adata, cluster_key, target=None, reference="siblings", device=None, baseline="zero",
    ...
    for s in states:
        ...
        att, dC = _attribute_one(enc, counts, tmask, rmask, device, baseline=baseline)
        cols[s] = att
        dcols[s] = dC
    ...
    return AttributionResult(pd.DataFrame(cols, index=genes), ranked,
                             {"reference": reference, "n_genes": len(genes), "baseline": baseline, "gate": gate},
